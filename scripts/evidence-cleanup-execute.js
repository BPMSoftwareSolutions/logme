...
function runsEvidenceCleanupExecuteCli() {
  ...
  const result = executesEvidenceCleanup(rootDir, plan, planHash, approvalRecord);

  if (result.verdict === 'ADVISORY_ONLY') {
    process.stdout.write(
      'Cleanup remains advisory only -- a deterministic gate did not pass:\n'
      + `Approval check: ${result.approvalResult.verdict}${result.approvalResult.reason ? ` (${result.approvalResult.reason})` : ''}\n`
      + `Run "npm run evidence:cleanup-approve -- --approved-by <name> --actions archive,delete" first.\n`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  process.stdout.write(
    ...
    + `Archived: ${result.executionResult.archivedRuns.length}\n`
    + `Deleted: ${result.executionResult.deletedRuns.length}\n`
